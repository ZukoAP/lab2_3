...
namespace graph {
    ...
    sequence::SequenceIterator<Pair<N, L>> UndirectedGraph<L, N>::neighbours(N node) {
        if (this->nodes->contains(node)) {
            ...
            for (unsigned int i = 0; i < nodesPtr->length(); ++i) {
                if (this->edges.count(std::make_pair(node, nodesPtr->getElement(i)->node))){
    ...
    bool UndirectedGraph<L, N>::isNeighbours(N node1, N node2) {
        return this->edges.count(std::make_pair(node1, node2)) || this->edges.count(std::make_pair(node2, node1));
    }

    template<typename L, typename N>
    L UndirectedGraph<L, N>::getLabel(N node1, N node2) {
        if (this->isNeighbours(node1, node2)) {
            if (this->edges.count(std::make_pair(node1, node2))) {
    ...
    void UndirectedGraph<L, N>::removeEdge(N node1, N node2) {
        if (this->edges.count(std::make_pair(node1, node2)) || this->edges.count(std::make_pair(node2, node1))) {
            if (node1 == node2) {
